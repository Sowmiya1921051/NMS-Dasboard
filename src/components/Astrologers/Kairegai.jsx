...
const Kairegai = () => {
  ...
  // Fetch data from the kairagai API
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://bank.infiscripts.com/kairagai.php');
  ...
  const handleAddAstrologer = async () => {
    ...
    try {
      ...
      if (result.status === 'success') {
        // Show newly uploaded image instantly
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-6">Kairagai Data</h1>
      ...
      {isInputVisible && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-lg shadow-lg w-[600px] h-[500px] overflow-auto">
            <h2 className="text-xl font-semibold mb-4">Add New Kairagai Expert</h2>
