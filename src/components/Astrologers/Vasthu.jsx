...
const Vastu = () => {
  ...
  // Fetch data from the vastu API
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://bank.infiscripts.com/vastu.php');
  ...
  const handleAddAstrologer = async () => {
    ...
    try {
      ...
      if (result.status === 'success') {
        ...
        alert('Vastu expert added successfully');
      } else {
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-6">Vastu Expert Data</h1>
      ...
      {isInputVisible && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-lg shadow-lg w-[600px] h-[500px] overflow-auto">
            <h2 className="text-xl font-semibold mb-4">Add New Vastu Expert</h2>
