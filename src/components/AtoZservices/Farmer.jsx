...
const Farmer = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch('https://bank.infiscripts.com/farmer.php');
    ...
    const handleAddHall = async () => {
        ...
        try {
            ...
            if (result.status === 'success') {
                ...
                alert('Error adding farmer: ' + result.message);
            }
        } catch (error) {
    ...
    return (
        <div className="p-6">
            <h1 className="text-2xl font-bold mb-6">Farmer Data</h1>
            <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {hallData.map((hall) => (
                    ...
                    >
                        ...
                        <p className="text-gray-700">Activity: {hall.activity}</p>
                    </div>
                ))}
            </div>
        </div>
...
// Farmer Input Component (Memoized to prevent re-renders)
const HallInput = memo(({ newHall, handleInputChange, handleImageUpload, handleAddHall, resetForm }) => (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white p-8 rounded-lg shadow-lg w-[600px] h-[500px] overflow-auto">
            ...
            {['name', 'phone_number', 'address', 'experience', 'activity'].map((field) => (
...
// Farmer Details Component (Memoized to prevent re-renders)
const HallDetails = memo(({ hall, close }) => (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white p-8 rounded-lg shadow-lg w-[500px] h-[300px] overflow-auto">
            ...
            {['name', 'phone_number', 'address', 'activity'].map((key) => (
