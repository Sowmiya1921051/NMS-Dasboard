...
const FlowerService = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://bank.infiscripts.com/flower.php');
  ...
  const handleAddHall = async () => {
    ...
    try {
      ...
      if (result.status === 'success') {
        ...
        alert('FlowerService added successfully');
        resetForm();
      } else {
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-6">Flower Service Data</h1>
      <button
...
const HallInput = memo(({ newHall, handleInputChange, handleImageUpload, handleAddHall, resetForm }) => (
  <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div className="bg-white p-8 rounded-lg shadow-lg w-[600px] h-[500px] overflow-auto">
      <h2 className="text-xl font-semibold mb-4">Add New Flower Service</h2>
