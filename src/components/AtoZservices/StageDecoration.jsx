...
const StageDecoration = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://bank.infiscripts.com/stagedecore.php');
  ...
  const handleAddHall = async () => {
    ...
    try {
      ...
      if (result.status === 'success') {
        ...
        alert('StageDecoration added successfully');
        resetForm();
      } else {
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-6">Stage Decoration Data</h1>
      <button
        className="mb-4 px-4 py-2 bg-blue-500 text-white rounded-lg"
        onClick={() => setIsInputVisible(true)}
      >
        Add Stage Decoration
      </button>
...
const HallInput = memo(({ newHall, handleInputChange, handleImageUpload, handleAddHall, resetForm }) => (
  <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div className="bg-white p-8 rounded-lg shadow-lg w-[600px] h-[500px] overflow-auto">
      <h2 className="text-xl font-semibold mb-4">Add New Stage Decoration</h2>
